...
class MediaRoutes extends BaseRoute {
	...
	protected function get_routes(): array {
		return array(
			...
			array(
				'route'    => '/attachment-by-url',
				'methods'  => 'GET',
				'callback' => array( $this, 'get_attachment_id_by_url' ),
				'args'     => array(
					'url' => array(
						'required'          => true,
						'type'              => 'string',
						'sanitize_callback' => 'esc_url_raw',
						'validate_callback' => array( $this, 'validate_attachment_url' ),
					),
				),
			),
		);
	...
	/**
	 * Validate that a parameter is a non-empty URL.
	 *
	 * @param mixed $value The value to validate.
	 * @return bool True if the value is a valid URL string.
	 */
	public function validate_attachment_url( $value ): bool {
	...
	/**
	 * Resolve an attachment ID from a URL pointing at an uploaded file.
	 *
	 * Returns 404 when the URL does not match any attachment in the current
	 * site's uploads (for example external URLs or URLs from another site).
	 * Handles URLs for any registered image size via `attachment_url_to_postid()`.
	 *
	 * @param WP_REST_Request<array{url: string}> $request The REST request object.
	 * @return WP_REST_Response|WP_Error The lookup result or an error.
	 */
	public function get_attachment_id_by_url( $request ) {
		$url = (string) $request->get_param( 'url' );
		...
		if ( $attachment_id <= 0 ) {
			return new WP_Error(
				...
				__( 'No attachment matches the given URL.', 'etch' ),
				array( 'status' => 404 )
			);
		}

		return new WP_REST_Response( array( 'id' => $attachment_id ), 200 );
	}
}

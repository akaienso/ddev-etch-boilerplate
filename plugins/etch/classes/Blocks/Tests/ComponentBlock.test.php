...
class ComponentBlockTest extends WP_UnitTestCase {
	...
	/**
	 * Helper: set up a component pattern post and its property definitions and render it.
	 * Returns the rendered HTML.
	 *
	 * @param string               $inner_content Pattern post content (blocks to render inside the component).
	 * @param array<int, array>    $properties    Property definitions.
	 * @param array<string, mixed> $instance_attrs Instance attributes override.
	 * @param string               $component_title Pattern post title.
	 * @return array{html: string, pattern_id: int}
	 */
	private function render_component_with_namespace(
		string $inner_content,
		array $properties = array(),
		array $instance_attrs = array(),
		string $component_title = 'Hero'
	): array {
		$pattern_id = $this->factory()->post->create(
			array(
				'post_type'    => 'wp_block',
		...
		update_post_meta( $pattern_id, 'etch_component_html_key', 'Hero' );
		...
		$block = $this->create_mock_block( 'etch/component', $attributes );
		$html = $this->component_block->render_block( $attributes, '', $block );
	...
	/**
	 * Component.id, component.name, and component.key resolve to the
	 * component's ref id, post title, and html key post meta respectively.
	 */
	public function test_component_namespace_exposes_metadata_fields() {
		$result = $this->render_component_with_namespace(
			'<!-- wp:etch/text {"content":"Id: {component.id} | Name: {component.name} | Key: {component.key}"} /-->',
			array(),
			array(),
			'My Hero'
		);
		$this->assertStringContainsString( 'Id: ' . $result['pattern_id'], $result['html'] );
		$this->assertStringContainsString( 'Name: My Hero', $result['html'] );
		$this->assertStringContainsString( 'Key: Hero', $result['html'] );
	}

	/**
	 * Component.defaults.<key> resolves to the leaf default value.
	 */
	public function test_component_namespace_exposes_defaults_for_top_level_prop() {
		$result = $this->render_component_with_namespace(
			'<!-- wp:etch/text {"content":"Default: {component.defaults.text}"} /-->',
			array(
				array(
					'key'     => 'text',
					'type'    => array( 'primitive' => 'string' ),
		...
		$this->assertStringContainsString( 'Default: Hello', $result['html'] );
	}

	/**
	 * Component.defaults for a class prop resolves style IDs into class names.
	 */
	public function test_component_namespace_resolves_class_prop_default_to_names() {
		update_option(
			'etch_styles',
			array(
				'hero-id' => array(
		...
		$result = $this->render_component_with_namespace(
			'<!-- wp:etch/text {"content":"Default: {component.defaults.classes}"} /-->',
			array(
				array(
					'key'     => 'classes',
					'type'    => array(
					...
					'default' => array( 'hero-id' ),
				),
			)
		);

		$this->assertStringContainsString( 'Default: hero', $result['html'] );

		delete_option( 'etch_styles' );
	}

	/**
	 * Component.defaults preserves group nesting.
	 */
	public function test_component_namespace_exposes_nested_group_defaults() {
		$result = $this->render_component_with_namespace(
			'<!-- wp:etch/text {"content":"Nested: {component.defaults.layout.width}"} /-->',
			array(
				array(
					'key'        => 'layout',
					'type'       => array(
		...
		$this->assertStringContainsString( 'Nested: 200px', $result['html'] );
	}
}

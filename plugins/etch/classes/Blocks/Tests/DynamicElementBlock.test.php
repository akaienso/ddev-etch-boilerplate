...
class DynamicElementBlockTest extends WP_UnitTestCase {
	...
	/**
	 * Helper to render a dynamic element block with given attributes and optional inner blocks.
	 *
	 * @param array $attributes   Block attributes.
	 * @param array $inner_blocks Optional inner blocks.
	 * @return string Rendered output.
	 */
	private function render_dynamic_element( array $attributes, array $inner_blocks = array() ): string {
		$block = $this->create_mock_block( 'etch/dynamic-element', $attributes, $inner_blocks );
		return $this->dynamic_element_block->render_block( $attributes, '', $block );
	}
	...
	public function test_dynamic_element_renders_with_tag_from_attributes() {
		$result = $this->render_dynamic_element(
			array(
	...
	public function test_dynamic_element_invalid_tag_defaults_to_div() {
		$result = $this->render_dynamic_element(
			array(
				'tag'        => 'div',
				'attributes' => array( 'tag' => 'invalid<>tag' ),
			)
		);
		$this->assertStringStartsWith( '<div', $result );
	}
	...
	public function test_dynamic_element_tag_from_component_props() {
		$result = $this->render_component(
			'<!-- wp:etch/dynamic-element {"tag":"div","attributes":{"tag":"{props.elementTag}","class":"test"}} /-->',
			array(
				array(
					'key'     => 'elementTag',
	...
	public function test_dynamic_element_tag_with_default_only() {
		$result = $this->render_component(
			...
			array() // No instance value, should use default
		);
		$this->assertStringStartsWith( '<section', $result );
		$this->assertStringEndsWith( '</section>', $result );
	}
	...
	public function test_dynamic_element_tag_instance_overrides_default() {
		$result = $this->render_component(
			...
			array( 'elementTag' => 'h2' ) // Instance value should override default
		);
	...
	public function test_dynamic_element_attributes_with_component_props() {
		$result = $this->render_component(
			'<!-- wp:etch/dynamic-element {"tag":"div","attributes":{"tag":"header","id":"{props.elementId}","class":"{props.className}"}} /-->',
			array(
				array(
					'key'     => 'elementId',
					'type'    => array( 'primitive' => 'string' ),
	...
	public function test_dynamic_element_nested_component_prop_shadowing() {
		...
		$result = $this->render_component(
			...
			array( 'elementTag' => 'h1' ) // Parent value
		);
	...
	/**
	 * Test dynamic element renders void element as self-closing when tag is br
	 */
	public function test_dynamic_element_renders_void_element_as_self_closing() {
		$result = $this->render_dynamic_element(
			array(
				'tag'        => 'div',
				'attributes' => array( 'tag' => 'br' ),
			)
		);

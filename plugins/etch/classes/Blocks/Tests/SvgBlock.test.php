...
class SvgBlockTest extends WP_UnitTestCase {
	...
	/**
	 * Create an SVG attachment in the uploads directory.
	 * Use a unique fill color per test so assertions can pinpoint which SVG rendered.
	 *
	 * @param string $filename   Filename to create under the uploads basedir.
	 * @param string $fill_color Fill color for the distinctive `<rect>` marker.
	 * @return int Attachment ID.
	 */
	private function create_svg_attachment( string $filename, string $fill_color ): int {
		$upload_dir = wp_get_upload_dir();
		$path       = $upload_dir['basedir'] . '/' . $filename;
		$content    = '<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60"><rect fill="' . $fill_color . '" width="60" height="60"/></svg>';
	...
	/**
	 * Render an etch/svg block with the given src string.
	 *
	 * @param string $src Value to set as the block's `src` attribute.
	 * @return string Rendered block markup.
	 */
	private function render_svg_with_src( string $src ): string {
		$attributes = array(
			'tag'        => 'svg',
			'attributes' => array( 'src' => $src ),
		);
		$block = $this->create_mock_block( 'etch/svg', $attributes );
		return $this->svg_block->render_block( $attributes, '', $block );
	}

	/**
	 * Happy path: numeric src resolves to an SVG attachment.
	 */
	public function test_svg_block_renders_attachment_svg_when_src_is_numeric_id_of_svg() {
		$attachment_id = $this->create_svg_attachment( 'attachment-id-test.svg', 'magenta' );

		$result = $this->render_svg_with_src( (string) $attachment_id );

		// The magenta marker proves the attached SVG was loaded (not the fallback).
		$this->assertStringContainsString( 'fill="magenta"', $result );
	}

	/**
	 * Interface boundary: numeric ID points to a non-SVG attachment (e.g. PNG).
	 */
	public function test_svg_block_renders_fallback_when_src_is_numeric_id_of_non_svg_attachment() {
		$attachment_id = wp_insert_attachment(
			array(
				'post_mime_type' => 'image/png',
				'post_title'     => 'not-an-svg.png',
				'post_status'    => 'inherit',
			)
		);
		...
		// viewBox="0 0 200 200" is unique to the Etch fallback SVG.
		$this->assertStringContainsString( 'viewBox="0 0 200 200"', $result );
	}

	/**
	 * Zero / exception case: numeric ID that does not resolve to any attachment.
	 */
	public function test_svg_block_renders_fallback_when_src_is_numeric_id_that_does_not_exist() {
		// 999999 is not a real attachment in a fresh test DB.
		$result = $this->render_svg_with_src( '999999' );

		$this->assertStringContainsString( 'viewBox="0 0 200 200"', $result );
	}

	/**
	 * Boundary: numeric ID surrounded by whitespace should still resolve.
	 */
	public function test_svg_block_renders_attachment_svg_when_src_is_numeric_id_with_whitespace() {
		$attachment_id = $this->create_svg_attachment( 'attachment-id-whitespace.svg', 'cyan' );

		$result = $this->render_svg_with_src( '  ' . $attachment_id . ' ' );

		// The cyan marker proves the padded ID was trimmed and resolved.
		$this->assertStringContainsString( 'fill="cyan"', $result );
	}
}

...
class SvgBlock {
	...
	public function render_block( $attributes, $content, $block = null ) {
		...
		$src = $this->resolve_src_for_svg_loader( $resolved_attributes['src'] ?? '' );
	...
	/**
	 * Resolve SVG src values before loading SVG content.
	 *
	 * Numeric src values are treated as WordPress attachment IDs. Only SVG
	 * attachments resolve to URLs; non-SVG and missing attachments resolve to
	 * an empty src so SvgLoader returns the Etch logo fallback.
	 *
	 * @param string $src SVG source attribute.
	 * @return string Source URL or empty fallback source.
	 */
	private function resolve_src_for_svg_loader( string $src ): string {
		$trimmed_src = trim( $src );
		if ( ! ctype_digit( $trimmed_src ) ) {
			return $src;
		}

		$attachment_id = (int) $trimmed_src;

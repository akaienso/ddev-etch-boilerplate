...
trait ACFData {
		...
		if ( ! Flag::is_on( 'RETURN_ACF_DYNAMIC_DATA' ) ) {
			switch ( $field_type ) {
				...
				case 'relationship':
					...
					return $this->return_format_post_object_field( $value, $return_format );
	...
	/**
	 * Check if need to call format_post_object_field or just return the raw value based on the given value.
	 *
	 * @param mixed $value The field value.
	 * @param mixed $return_format Type of format to be returned (object or id).
	 * @return mixed Return array if return_format is object or return the raw value.
	 */
	private function return_format_post_object_field( $value, $return_format = '' ): mixed {
		if ( ! Flag::is_on( 'RESPECT_ACF_RETURN_FORMAT' ) ) {
			return $this->format_post_object_field( $value );
		}

		$return_format = is_string( $return_format ) ? $return_format : '';
